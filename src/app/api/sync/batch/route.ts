...
import { getShopifyProduct, getShopifyApiSettings, updateInventoryLevel } from "@/lib/shopify";
...
/**
 * Function to sync a product from Shopify to Nhanh.vn
 * This is a simplified implementation that only syncs inventory
 */
async function syncProductToNhanh(shopifyId: string): Promise<SyncResult> {
  try {
    ...
    if (!mapping) {
      return {
        success: false,
        error: `No mapping found for product with shopifyId: ${shopifyId}`
      };
    }
    ...
    try {
      ...
      return {
        success: false,
        error: `Error parsing nhanhData: ${(e as Error).message}`
      };
    }

    // Get API settings
    const settings = await syncService.getApiSettings();
    ...
    try {
      // Call syncInventory with proper parameters: product object, parsed nhanhData, settings, and username
      const result = await syncService.syncInventory(product, nhanhData, settings, 'system');
      
      // Kiểm tra result và trả về phản hồi phù hợp
      if (result && result.skipped) {
        console.log(`[BATCH SYNC] Skipped syncing inventory for product ${shopifyId}: ${result.reason}`);
        return {
          success: true
        };
      }
      ...
      if (result && 'success' in result && result.success) {
        console.log(`[BATCH SYNC] Successfully synced inventory for product ${shopifyId}, quantity: ${result.inventoryQuantity}`);
        return {
          success: true
        };
      }
      ...
      return {
        success: false,
        error: error.message || 'Error syncing inventory'
      };
    }
  } catch (error) {
    console.error(`Error syncing product to Nhanh: ${(error as Error).message}`);
    return {
      success: false,
...
export async function GET(req: NextRequest) {
  try {
    ...
    const batchLog = await prisma.syncLog.findFirst({
      where: {
        details: {
          contains: batchId
        }
    ...
    const jobIds = batchDetails.jobs?.map((job) => job.jobId) || [];
    ...
    const jobStatuses = await Promise.all(jobIds.map(async (jobId: string) => {
      try {
        ...
        return { jobId, status: 'error', error: (error as Error).message };
      }
    }));
    ...
    const stats = {
      total: jobIds.length,
      completed: jobStatuses.filter(job => job.status === 'completed').length,
      failed: jobStatuses.filter(job => job.status === 'failed').length,
      waiting: jobStatuses.filter(job => ['waiting', 'delayed'].includes(job.status as string)).length,
      active: jobStatuses.filter(job => job.status === 'active').length,
      progress: Math.floor((jobStatuses.filter(job => job.status === 'completed').length / (jobIds.length || 1)) * 100)
    };
